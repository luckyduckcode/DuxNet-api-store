...
pub struct ApiState {
    ...
    pub api_keys: Arc<HashMap<String, String>>, // api_key -> user_did mapping
}
...
async fn authenticate_api_key(
    ...
    // In a real implementation, you'd check the request path
    
    if let Some(auth_header) = headers.get("Authorization") {
        if let Ok(auth_str) = auth_header.to_str() {
            if auth_str.starts_with("Bearer ") {
                let api_key = &auth_str[7..]; // Remove "Bearer " prefix
...
pub async fn start_api_server(port: u16) -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("🔑 Demo API Keys: demo-api-key-123, admin-api-key-456");
    info!("📝 Usage: curl -H 'Authorization: Bearer demo-api-key-123' http://localhost:8081/api/status");
